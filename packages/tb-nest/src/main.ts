 ...
 * TB Backend - Runs on port 3001 with tb-db PostgreSQL database
 */
async function bootstrap(): Promise<void> {
  const appLogger = new AppLogger('TB-Backend');
  appLogger.log('Starting TB Backend application...');

  // Validate required configuration before creating the NestJS app
  try {
    appLogger.log('Validating TB environment configuration...');
    ConfigUtil.validateRequiredConfig();
    appLogger.log('TB environment configuration validated successfully');
  } catch (error) {
    appLogger.error(
      'Failed to start TB application due to missing configuration:',
  ...
  // TB Backend uses TB_PORT (default 3001) or PORT fallback
  const port = parseInt(
    process.env.TB_PORT || process.env.PORT || (useHttps ? '3443' : '3001'),
    10,
  );
  const host = process.env.TB_HOST || process.env.HOST || '0.0.0.0';
  ...
  if (useHttps && isDev) {
    ...
    try {
      ...
      appLogger.log('TB NestJS application created with HTTPS enabled');
    } catch (error) {
      appLogger.error(
        `TB Backend failed to load SSL certificates: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error instanceof Error ? error.stack : undefined,
      );
      throw error;
    }
    ...
    appLogger.log('TB NestJS application created with HTTP');
  }
  ...
  appLogger.log(
    `🚀 TB Backend is running on: ${protocol}://${host}:${port} ${
      isDev ? '(Development)' : '(Production)'
    }`,
  );
  appLogger.log(`📚 TB API Docs available at: ${protocol}://${host}:${port}/docs`);
  appLogger.log(`🗄️  TB Database: ${process.env.TB_POSTGRES_HOST}:${process.env.TB_POSTGRES_PORT}/${process.env.TB_POSTGRES_DATABASE}`);
}

bootstrap().catch((err) => {
  console.error('TB Backend bootstrap failed:', err);
  process.exit(1);
});

@Catch()
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly logger = new AppLogger('TB-GlobalExceptionFilter');
