    ...
    <div>
          ...
          <Table>
            ...
            <TableBody>
              {currentData.map((call) => (
                <TableRow key={call.id} className="hover:bg-muted/50 cursor-pointer">
                  <Link href={`/lab/${call.id}`} className="contents">
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold">{call.name}</span>
                        <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">v{call.promptVersion}</span>
                      </div>
                    </TableCell>
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-muted-foreground">
            Showing {startIndex + 1}-{Math.min(endIndex, data.length)} of {data.length} named calls
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
