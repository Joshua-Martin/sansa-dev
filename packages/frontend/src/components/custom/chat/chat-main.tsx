...
import { usePromoStore } from '../promo/use-promo-store';
...
export const ChatMain: React.FC<ChatMainProps> = ({ className }) => {
  ...
  const { setPromoPreview, setLoading } = usePromoStore();
  ...
  const handleSendMessage = async (message: string, uiState?: string) => {
    ...
    if (isPromo) {
      ...
      // Send message with 2-second delay for websocket call (but optimistic UI update happens immediately)
      await sendMessage(message, currentUiState, 4000, () => {
        // Turn off thinking indicator when message is completed or fails
        setIsThinking(false);
      });
      ...
      // Normal behavior without promo timeout
  ...
  return (
    <div
      className={cn(
        'max-h-screen min-h-screen h-full flex relative bg-background',
        className
      )}
    >
      {/* Main Chat Interface */}
