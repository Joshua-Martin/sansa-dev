...
async function bootstrap(): Promise<void> {
  const appLogger = new AppLogger('Application');
  appLogger.log('Starting application...');

  // Validate required configuration before creating the NestJS app
  try {
    appLogger.log('Validating environment configuration...');
    ConfigUtil.validateRequiredConfig();
    appLogger.log('Environment configuration validated successfully');
  } catch (error) {
    appLogger.error(
      'Failed to start application due to missing configuration:',
  ...
  const port = parseInt(process.env.PORT || (useHttps ? '3443' : '3000'), 10);
  ...
  if (useHttps && isDev) {
    ...
    try {
      ...
      appLogger.log('NestJS application created with HTTPS enabled');
    } catch (error) {
      appLogger.error(
    ...
    appLogger.log('NestJS application created with HTTP');
  }
  ...
  appLogger.log(
    `🚀 Application is running on: ${protocol}://${host}:${port} ${
...
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly logger = new AppLogger('GlobalExceptionFilter');
