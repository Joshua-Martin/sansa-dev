  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleChatMessage(
    ...
    try {
      ...
      // 🔍 LOG USER MESSAGE: Clear logging of incoming user message
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.is_final) {
          ...
          // 🔍 LOG COMPLETE LLM RESPONSE: Clear logging of final response
          this.logger.log(
            `🤖 [LLM_RESPONSE][BASE_RESPONSE] Final Content: ${accumulatedContent}, User: ${userId}, Thread: ${threadId}, Event ID: ${eventId || 'NONE'}`,
          );
