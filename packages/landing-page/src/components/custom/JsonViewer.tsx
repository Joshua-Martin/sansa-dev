...
const JsonViewer: React.FC<JsonViewerProps> = ({ data, title, price, latency, onMeasure, shouldScale = false }) => {
   ...
   */
  const computeRelativePoint = (el: HTMLElement | null, kind: 'centerBottom' | 'topCenter' | 'bottomCenter' | 'rightCenter') => {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      onMeasure({
        messageKeyCenterBottom: computeRelativePoint(messageKeyRef.current, 'centerBottom'),
        messageValueCenterBottom: computeRelativePoint(messageValueRef.current, 'centerBottom'),
        priceTopCenter: computeRelativePoint(priceRef.current, 'topCenter'),
        priceBottomCenter: computeRelativePoint(priceRef.current, 'bottomCenter'),
        latencyRightCenter: computeRelativePoint(latencyRef.current, 'rightCenter'),
        containerRect,
      });
    };
    measure();
  ...
  const formatJson = (obj: JsonValue, indent = 0, propertyName?: string, nodePath = 'root'): React.ReactElement[] => {
    const elements: React.ReactElement[] = [];
    const indentStr = '  '.repeat(indent);

    if (obj === null) {
      elements.push(<span key={`null-${nodePath}`} className="text-blue-400">null</span>);
    } else if (typeof obj === 'boolean') {
      elements.push(<span key={`bool-${nodePath}`} className="text-blue-400">{obj.toString()}</span>);
    } else if (typeof obj === 'number') {
      elements.push(<span key={`num-${nodePath}`} className="text-green-400">{obj}</span>);
      ...
      if (obj.length > maxLineLength) {
        ...
        children.push(<span key={`str-start-${nodePath}`} className="text-yellow-400">&quot;</span>);
        lines.forEach((line, index) => {
          if (index > 0) {
            ...
            children.push(<span key={`indent-${nodePath}-${index}`} className="text-gray-600">{continuationIndent}</span>);
          }
          children.push(<span key={`line-${nodePath}-${index}`} className="text-yellow-400">{line}</span>);
        });
        children.push(<span key={`str-end-${nodePath}`} className="text-yellow-400">&quot;</span>);
        if (propertyName === 'message') {
          elements.push(
            <span key={`msg-val-wrap-${nodePath}`} data-json-value="message">{children}</span>
          );
        } else {
          elements.push(<span key={`str-wrap-${nodePath}`}>{children}</span>);
        }
      } else {
        const short = <span key={`str-inner-${nodePath}`} className="text-yellow-400">&quot;{obj}&quot;</span>;
        if (propertyName === 'message') {
          elements.push(
            <span key={`msg-val-${nodePath}`} data-json-value="message">{short}</span>
          );
        } else {
          elements.push(short);
        }
      ...
      elements.push(<span key={`array-start-${nodePath}`} className="text-gray-300">[</span>);
      if (obj.length > 0) {
        elements.push(<br key={`br-open-${nodePath}`} />);
        obj.forEach((item, index) => {
          elements.push(<span key={`indent-${nodePath}-${index}`} className="text-gray-600">{indentStr}  </span>);
          const itemElements = formatJson(item, indent + 1, undefined, `${nodePath}[${index}]`);
          elements.push(...itemElements);
          if (index < obj.length - 1) {
            elements.push(<span key={`comma-${nodePath}-${index}`} className="text-gray-300">,</span>);
          }
          elements.push(<br key={`br-item-${nodePath}-${index}`} />);
        });
        elements.push(<span key={`indent-end-${nodePath}`} className="text-gray-600">{indentStr}</span>);
      }
      elements.push(<span key={`array-end-${nodePath}`} className="text-gray-300">]</span>);
    } else if (typeof obj === 'object') {
      // At this point, obj is guaranteed to be a Record<string, JsonValue>
      const record = obj as Record<string, JsonValue>;
      elements.push(<span key={`obj-start-${nodePath}`} className="text-gray-300">{'{'}</span>);
      ...
      if (keys.length > 0) {
        ...
        keys.forEach((key, index) => {
          ...
          if (isMessageProperty && shouldScale) {
            ...
            // Add indentation
            propertyElements.push(<span key={`indent-${nodePath}-${key}`} className="text-gray-600">{indentStr}  </span>);

            // Add key
            propertyElements.push(
              <span key={`key-${nodePath}-${key}`} ref={messageKeyRef} data-json-key="message" className="text-purple-400">&quot;{key}&quot;</span>
            );

            // Add colon
            propertyElements.push(<span key={`colon-${nodePath}-${key}`} className="text-gray-300">: </span>);

            // Add value elements - pass undefined for propertyName so message value doesn't get individual styling
            const valueElements = formatJson(record[key], indent + 1, undefined, `${nodePath}.${key}`);
            propertyElements.push(...valueElements);

            // Add comma if not last
            if (index < keys.length - 1) {
              propertyElements.push(<span key={`comma-${nodePath}-${key}`} className="text-gray-300">,</span>);
            }
            ...
            elements.push(<span key={`indent-${nodePath}-${key}`} className="text-gray-600">{indentStr}  </span>);
            ...
            if (isMessageProperty) {
              ...
              elements.push(<span key={`key-${nodePath}-${key}`} className="text-purple-400">&quot;{key}&quot;</span>);
            }

            elements.push(<span key={`colon-${nodePath}-${key}`} className="text-gray-300">: </span>);

            const valueElements = formatJson(record[key], indent + 1, key, `${nodePath}.${key}`);
            elements.push(...valueElements);

            if (index < keys.length - 1) {
              elements.push(<span key={`comma-${nodePath}-${key}`} className="text-gray-300">,</span>);
            }
          }
      ...
      elements.push(<span key={`obj-end-${nodePath}`} className="text-gray-300">{'}'}</span>);
    }
  ...
  return (
    <div ref={containerRef} className="bg-background rounded-lg border border-border overflow-visible">
      {(title || price || latency) && (
        <div className="bg-muted px-3 py-2 border-b border-border flex items-center justify-between gap-3">
          {title && (
            <span className="text-sm font-medium text-muted-foreground">{title}</span>
          )}
          <div className={`mr-4 flex items-center ${shouldScale ? 'gap-8' : 'gap-2'}`}>
            {price && (
              <span ref={priceRef} className={`text-xs text-foreground/80 bg-background border border-border rounded px-2 py-0.5 ${shouldScale ? 'shadow-lg border-primary' : ''}`} data-json-header="price"
                style={{ transform: shouldScale ? 'scale(1.2)' : 'scale(1)' }}
              >{price}</span>
            )}
            {latency && (
              <span ref={latencyRef} className={`text-xs text-foreground/80 bg-background border border-border rounded px-2 py-0.5 ${shouldScale ? 'shadow-lg border-primary' : ''}`} data-json-header="latency"
                style={{ transform: shouldScale ? 'scale(1.2)' : 'scale(1)' }}
              >{latency}</span>
            )}
          </div>
